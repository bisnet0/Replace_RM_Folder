(function () {
    ...
    function EduLoginController($sce,
        ...
        function registrarEsqueciSenha() {
            ...
            var HttpRequest = $totvsresource.REST(urlServicos, { method: 'RecoverPassword' }),
                paramsRequest = {},
                url;
        ...
        function verificarSessaoExpirada() {
            ...
            if ($window.location.href.includes('sessaoexpirada')) {
                $timeout(function (){
                    totvsNotification.notify({
                        type: 'warning',
                        title: i18nFilter('l-Atencao'),
                        detail: i18nFilter('l-sessao-expirada')
                    });
                }, 2000);
            }
        }
        ...
        function getUrlParam(name) {
            ...
            //Em alguns casos a KEY continha o caractér '#'
            //sendo cortado apartir dele pela expressão regular e impedindo a autenticação no server.
            if(name === 'K')
            regex = '=([^&]*)';

            var results = new RegExp('[\\?&]' + name + regex).exec($location.$$hash);
        ...
        function verificarExibirFormularioAlterarSenha() {
            ...
            if (isChangePass !== -1) {
                ...
                var user = sessionStorage.getItem('user_change_pass');
                if (user) { 
                    self.usuario = atob(user);
                }
        ...
        function registrarNovaSenha() {
            ...
            var HttpRequest = $totvsresource.REST(urlServicos, { method: 'RegisterNewPassword' }),
                paramsRequest = {};

            paramsRequest.Key = self.keyTrocaSenha;
            paramsRequest.NovaSenha = self.novaSenha;
            paramsRequest.ConfirmacaoSenha = self.novaSenha2;
            paramsRequest.Alias = self.aliasRecover;

            HttpRequest.TOTVSPost({}, paramsRequest, function (result) {
        ...
        function alterarSenha() {
            ...
            var HttpRequest = $totvsresource.REST(urlServicos, { method: 'ChangePassword' }),
                paramsRequest = {};
        ...
        function externalLoginCallback() {
            ...
            new Oidc.OidcClient(settings).processSigninResponse()
